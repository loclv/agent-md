...
pub enum MarkdownBlock {
    Frontmatter(String),
    Heading { level: u32, text: String, raw: String },
    CodeBlock { language: Option<String>, content: String, raw: String },
    Table { header: String, separator: String, rows: Vec<String>, raw: String },
...
pub fn parse(content: &str) -> ParsedMarkdown {
    ...
    while i < lines.len() {
        ...
        if trimmed.starts_with("```") {
            if !in_code_block {
                ...
                blocks.push(MarkdownBlock::CodeBlock {
                    language: if code_block_lang.as_deref() == Some("") { None } else { code_block_lang.clone() },
        ...
        // List Item
        if let Some(_) = crate::rules::detect_list_item(trimmed) {
            ...
            while i < lines.len() {
                ...
                if crate::rules::detect_list_item(current_trimmed).is_some() || current_line.starts_with(' ') || current_line.starts_with('\t') {
            ...
            blocks.push(MarkdownBlock::List { items, raw: list_raw });
            continue;
        }

        // Table (simple detection: line contains |)
        if trimmed.starts_with('|') || (trimmed.contains('|') && i + 1 < lines.len() && lines[i+1].trim().contains('|') && lines[i+1].trim().contains('-')) {
