...
Table cells are auto-formatted — bold markers are stripped during `agent-md fmt` so the resulting table contains plain text. Inline code within table cells and throughout markdown (such as `code block: `let a = 1;``) is preserved unchanged without formatting modifications, including spaces inside backticks.
...
*Note: This rule is configurable via `.agent-md.json` (or `.markdownlint.json`). If `"blanks-around-headings": false` is specified, the linter will not enforce blank lines around headings, and the formatter will not auto-add spaces between headings and other content.*
